...
from .functions import train_test_data, train_test_data_dt
from .OwnDescent import OwnDescent
from .helperFunctions import fixSparseStructure_Iris, loadModel
from .DataLoader import load_data_mnist, load_config, load_data_iris
import pickle
import os
import time
...
def continuation_methodIris(X_train_ir, y_train_ir, X_test_ir, y_test_ir):
    ...
    # Store number of result files for naming reasons
    # Parent Directory path
    parent_dir = "C:\\Users\\Augustina\\OneDrive - mail.uni-paderborn.de\\Documents\\PhD\\Codes\\Continuation_method_project\\MNIST\\Results"
    directory_names = [dir for dir in os.walk(parent_dir)]
    directory_names = [dir[0] for dir in directory_names[1::]]
    # L number of result files
    L = len(directory_names)
    ...
    for k_ in range(n_continuation):
        ...
        #fixSparseStructure_Iris(model)
        #loadModel(model, "C:\\Users\\Augustina\\OneDrive - mail.uni-paderborn.de\\Documents\\PhD\\Codes\\Continuation_method_project\\MNIST\\Results\\Results_iris0\\Weights", 1)

        #**** Initialize parameters ****#
        params = model.parameters()
        learning_rate = 1e-2
        shrinkage_rate = 5e-4
        optimi = OwnDescent(params, lr=learning_rate, sr=shrinkage_rate, alpha = 4)
...
#****************MNIST DATA SET ************************#
def continuation_methodMNIST_deterministic(X_train_m,y_train_m, X_test_m,y_test_m ):
    ...
    for k_ in range(n_continuation):
        ...
        learning_rate = 1e-2
        shrinkage_rate = 5e-4
        optimi = OwnDescent(params, lr=learning_rate, sr=shrinkage_rate, alpha = 2)
        n_pgrad = config["n_pareto_grad"]
        n_pprox = config["n_pareto_prox"]
        n_pred = config["n_predictor"]
        n_cor = config["n_corr"]
    ...
    # Directory
    directory = "Results_det_0" #%s" % (k_ + L)
...
def continuation_methodMNIST_stochastic(X_train_m,y_train_m, X_test_m,y_test_m ): 
    ...
    for k_ in range(n_continuation):
        ...
        L1Norm_start, train_loss_start,test_loss_start, train_acc_all0, test_acc_all0   = train_test_data(type1, X_train, y_train, X_test, y_test, model=model, optimizer=optimi,
        ...
        L1Norm_pred_grad_all, train_loss_pred_grad_all, L1Norm_corr_grad_all, train_loss_corr_grad_all, train_acc_all1, test_loss_corr_grad_all, test_acc_all1  = train_test_data(type2, X_train, y_train,X_test, y_test,
                                                                                                                                                        model=model, optimizer=optimi, loss= loss ,n_pareto_grad=n_pgrad, n_pareto_prox=n_pprox,
        ...
        L1Norm_pred_prox_all, train_loss_pred_prox_all, L1Norm_corr_prox_all, train_loss_corr_prox_all, train_acc_all2, test_loss_corr_prox_all, test_acc_all2  = train_test_data(type3, X_train, y_train,X_test, y_test,
                                                                                                                                                            model=model, optimizer=optimi, loss= loss, n_pareto_grad=n_pgrad, n_pareto_prox=n_pprox,
    ...
    # Directory
    directory = "Results_sto_1" # s%" (k_ + L)
...
# *********Train for deterministic Mnist setting ************

#L1Norm_all,train_loss_all, test_loss_all, accuracy = continuation_methodMNIST_deterministic(X_train_m,y_train_m, X_test_m,y_test_m )

# **********Train for stochastic mnist setting ****************
